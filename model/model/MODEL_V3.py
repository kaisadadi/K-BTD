...
class JudgeNet(nn.Module):
    ...
    def word_level_score(self, T, Cm):
        ...
        col_S = self.col_softmax(S)  #importance measurement
    ...
    def forward(self, T, Cm, Fn, Am):
        ...
        return self.lambd * s1 + (1 - self.lambd) * s2, s1, s2, xi
...
class FusionNet(nn.Module):
    ...
    def forward(self, Cm, Fn, Am, s1, s2):
        alpha = torch.bmm(Fn.unsqueeze(1), Cm.transpose(0, 2, 1))
        beta = self.softmax(alpha.squeeze(1))
        inject_sum = torch.sum(beta.unsqueeze(2).repeat(1, 1, Cm.shape[2]) * Cm, dim = 1)
        Fm = Fn + (s2 / (s1 + s2)) * Am + (s1 / (s1 + s2)) * inject_sum
        return Fm
...
class MODEL_V3(nn.Module):
    def __init__(self, config):
        ...
        self.data = json.load(open("__YOUR_PATH__/syp_dict_0.5.json", "r"))
    ...
    def Node_loss(self, node, T, Fn, depth, target, criterion, mode, optimizer = None):
        ...
        prediction, s1, s2, xi = self.judge_net.forward(T.unsqueeze(0).repeat(bs, 1, 1), encoded_symptom, Fn.repeat(bs, 1), summed_output)
        Fm = self.fusion_net(encoded_symptom, Fn.repeat(bs, 1), summed_output, s1, s2)
